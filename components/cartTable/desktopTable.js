      ...
      <table className="table">
         <thead>
            <tr>
               ...
               <th scope="col" style={{display: 'flex', justifyContent: 'flex-end'}}>Total</th>
            </tr>
         </thead>
         <tbody>
            {cart.map((item, index) => {
               ...
               <tr key={`cart-item-${index}`}>
                  ...
                  <td>
                  {
                     ...
                     <div className="mt-2" style={{display: 'flex', flexWrap: 'wrap', justifyContent: 'center'}}>
                        ...
                        <p style={{ marginLeft: '10px'}}>${(config.products[item.number].price - 
                              (config.products[item.number].price * item.discount)).toFixed(2)}</p>
                     </div>
                     :
                     <p className="mt-2" style={{display: 'flex', justifyContent: 'center'}}>${config.products[item.number].price}</p>                  
                  }
                  </td>
                  <td>
                     <form onSubmit={(event) => update(event, item)} 
                        ...
                        <input
                           style={{width: '50px', fontSize: 'medium', marginRight: '10px', textAlign: 'center'}}
                           name="quantity"
                           type="number"
                  ...
                  <td>
                     {
                        item.quantity > 1 ? 
                        <div className="mt-2" style={{display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-end'}}>
                           <p><s>${(config.products[item.number].price * item.quantity).toFixed(2)}</s></p>
                           <p style={{ marginLeft: '10px'}}>${((config.products[item.number].price * item.quantity) - 
                                 ((config.products[item.number].price * item.quantity) * item.discount)).toFixed(2)}</p>
                        </div>
                        :
                        <p className="mt-2" style={{display: 'flex', justifyContent: 'flex-end'}}>${(config.products[item.number].price * item.quantity).toFixed(2)}</p>                  
                     }
                  </td>
               </tr>
               )

  ...
  return (
    <nav className="navbar navbar-expand-lg navi">
      <div className="container">
        <button
          className="mobile-nav nav-burger-btn"
        ...
        <div className="navbar-nav desktop-nav">
          {config.header.links.map(({ href, label }, index) => (
            <Link href={href} key={`navlink-${index}`}>
              <a className="nav-link a-link">{label}</a>
            </Link>
          ))}
        </div>
      ...
      <AnimateHeight duration={500} height={icon ? 0 : "auto"}>
        <div className="navbar-nav">
          {config.header.links.map(({ href, label }, index) => (
            <Link href={href} key={`navlink-${index}`}>
              <a className="nav-link a-link" onClick={() => setIcon(!icon)}>
                {label}
              </a>
            </Link>
          ))}
      ...
      <style jsx>{`
        ...
        :global(body) {
          margin: 0;
          font-family: -apple-system, Work Sans, BlinkMacSystemFont, Avenir Next,

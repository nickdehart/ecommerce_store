    ...
    fetch("/api/review", {
      ...
      .then(response => {
        ...
        if (response.status === 200)
          Swal.fire("Success!", "Thank you for writing a review.", "success");
        else
          Swal.fire({
            icon: "error",
            title: "Oops...",
  ...
  return (
    <div className="container">
      <div className="review-actions">
        ...
        <button onClick={() => setWrite(!write)}>Write a Review</button>
      </div>
      <form
        onSubmit={handleSubmit}
        style={{ display: write ? "block" : "none" }}
      >
        <div className="row">
      ...
      {data.length > 1 ? (
        <>
          {size && size.width > 767 && (
            <DesktopTable dataObj={dataObj} items={items} getStars={getStars} />
          )}
          {size && size.width < 768 && (
            <MobileTable dataObj={dataObj} items={items} getStars={getStars} />
          )}

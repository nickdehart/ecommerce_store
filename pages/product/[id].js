    ...
    fetch(`/api/review/meta`)
      ...
      .then(data => {
        for (var i = 0; i < data.length; i++) {
          if (data[i]._id === query.id) this.setState({ meta: [data[i]] });
        }
      })
      .catch(error => console.error(error));
  }
      ...
      <>
        <div className="container product">
          <div className="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 my-3">
            <Image
              src={`${product.assets}${product.images[activeIndex]}`}
              alt="Product"
              ...
              {product.images.map((item, index) => {
                return (
                  <Image
                    ...
                    className={`mx-2 p-2 thumbnail-img ${activeIndex ===
                      index && "thumbnail-selected"}`}
                    width={110}
                    height={110}
                  />
                );
          ...
          <div className="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 my-3">
            ...
            <p className="price mt-2">
              {product.multiplier && (
                <s>{`$${Math.floor(product.price * product.multiplier)}.99`}</s>
              )}
              ${product.price}
            </p>
            ...
            <form onSubmit={e => this.addToCart(e, product)}>
              <div className="form-group mb-3">
                ...
                />
                <Button variant="button" fullWidth>
                  Add To Cart
                </Button>
                {/* <PaypalButton
                cart={[{
